...
PDF = sys.argv[1] if len(sys.argv) > 1 else "/tmp/generated.pdf"
...
login = req("POST", "/auth/login", {"email": "admin@ai-gateway.local", "password": "admin123"})
...
for _ in range(80):
    ...
    if job.get("status") in ("SUCCESS", "FAILED", "SKIPPED"): break
    time.sleep(3)
...
for _ in range(80):
    ...
    if doc.get("status") in ("READY", "INDEXED", "SKIPPED_QUALITY", "FAILED") or doc.get("embeddingStatus") == "INDEXED":
        break
    time.sleep(3)
print(json.dumps({"job": job.get("status"), "status": doc.get("status"), "emb": doc.get("embeddingStatus"), "chunks": doc.get("chunkCount"), "chars": doc.get("parserChars")}, indent=2))
